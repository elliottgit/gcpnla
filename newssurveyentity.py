...
target_table_id = 'SMNewsSurveyResults'
TIMEOUT = 30
...
#query_job = bq.query('SELECT ' + source_target_table_id + ',' + source_target_table_column_name + ' FROM `' + PROJECT_ID + '.' + DATASET_ID + '.' + source_table_id + '` WHERE ' + source_target_table_column_name + ' is not null LIMIT 700')
query_job = bq.query('SELECT ' + source_target_table_id + ',' + source_target_table_column_name + ' FROM `' + PROJECT_ID + '.' + DATASET_ID + '.' + source_table_id + '` LIMIT 600 OFFSET 1200')
